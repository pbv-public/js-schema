# JS Schema Library <!-- omit in toc -->
The JS Schema library allows developers to quickly construct
...
## Getting JSON Schema
Check if an object is a Schema object by checking `isSchema`. The JSON schema
it describes can be generated by calling `jsonSchema()` on a Schema object:
```javascript
if (schema.isSchema) {
...
## Export schemas
Schemas can be extended to support exporting to custom schemas. It can be done via the `export` method which uses a visitor pattern. A custom exporter needs to implement the follow interface:
```javascript
...
For libraries that accept a fluent-schema object as the parameter (e.g.
fastify), you may pass Schema objects instead. Schema implements
fluent-schema's `isFluentSchema` and `valueOf()` APIs to achieve compatibility.
...
- Schema object is copied using `copy()`
